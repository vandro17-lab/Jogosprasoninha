...
import {
  ...
  Pencil, Upload, Check, Video, Send,
} from 'lucide-react'
...
const WHATSAPP_MESSAGE = `Oi, mãezinha! 💖

Nem tudo sai perfeito como a gente imagina, né? 😅
Se o QR Code não abriu direitinho no seu celular, não tem problema nenhum.

O seu presente continua aqui, feito com muito carinho pra você. 🎁✨

É só clicar nesse link para abrir a sua homenagem:

https://soninhamemo.vercel.app/sua-homenagem

Espero que você goste, porque tudo foi preparado com muito amor. 💐
Te amo muito! ❤️`
...
export default function AdminDashboard() {
  ...
  const [soniaPhone, setSoniaPhone] = useState('')
  ...
  function handleSoniaPhoneChange(val: string) {
    setSoniaPhone(val)
    localStorage.setItem('sonia_phone', val)
  }

  function handleSendWhatsApp() {
    const digits = soniaPhone.replace(/\D/g, '')
    if (!digits) return
      ...
      <div className="max-w-2xl mx-auto px-4 py-5 flex flex-col gap-4">
        ...
        >
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="font-semibold text-gray-800 text-sm">Enviar link de homenagem</h2>
          </div>
          <p className="text-xs text-gray-400 mb-4 ml-10">
            Se o QR Code não funcionar, abra o WhatsApp com o link já na mensagem.
          </p>
          ...
          <input
            ...
            value={soniaPhone}
            onChange={(e) => handleSoniaPhoneChange(e.target.value)}
            className="w-full rounded-xl px-4 py-2.5 text-sm text-gray-700 border outline-none transition-all mb-4"
          ...
          <button
            ...
            disabled={!soniaPhone.replace(/\D/g, '')}
            className="w-full flex items-center justify-center gap-2.5 py-3 px-4 rounded-xl font-semibold text-sm transition-all active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed"
            style={{
              background: soniaPhone.replace(/\D/g, '')
                ? 'linear-gradient(135deg, #C9A84C 0%, #a8872e 100%)'
                : 'rgb(243,244,246)',
              color: soniaPhone.replace(/\D/g, '') ? '#fff' : '#9ca3af',
              boxShadow: soniaPhone.replace(/\D/g, '')
          ...
          >
            ...
            Enviar link de homenagem pelo WhatsApp
          </button>
        </div>
